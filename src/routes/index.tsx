...
import {
  appointments, payments, recalls, reviews, newLeads,
...
function Dashboard() {
  ...
  const todayCollected = payments
    .filter((p) => p.dueDate >= "2026-05-15" && p.status === "paid")
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <>
            <Button variant="outline" size="sm" onClick={() => toast.success("Nouveau patient ajouté")}>
              <Users className="size-4" /> Nouveau patient
            </Button>
            <Button size="sm" onClick={() => toast.success("Rendez-vous créé")}>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>Planning du cabinet — 18 mai 2026</CardDescription>
            </div>
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-2 gap-2">
            <Button variant="outline" className="h-auto flex-col py-4 gap-1.5" onClick={() => toast.success("WhatsApp envoyé")}>
              <MessageCircle className="size-5 text-success" />
              <span className="text-xs font-medium">Envoyer WhatsApp</span>
            </Button>
            <Button variant="outline" className="h-auto flex-col py-4 gap-1.5" onClick={() => toast.success("Marqué comme payé")}>
              <CheckCircle2 className="size-5 text-primary" />
              <span className="text-xs font-medium">Marquer payé</span>
            </Button>
            <Button variant="outline" className="h-auto flex-col py-4 gap-1.5" onClick={() => toast.success("Lien d'avis envoyé")}>
              <Star className="size-5 text-warning" />
              <span className="text-xs font-medium">Envoyer avis</span>
            </Button>
            <Button variant="outline" className="h-auto flex-col py-4 gap-1.5" onClick={() => toast.success("Rappel planifié")}>
            ...
            {unpaid.slice(0, 4).map((p) => (
              <div key={p.id} className="flex items-center justify-between gap-3 rounded-xl border bg-background/50 p-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Button size="icon" variant="ghost" onClick={() => toast.success(`Rappel envoyé à ${p.patient}`)}>
            ...
            {upcomingRecalls.map((r) => (
              <div key={r.id} className="flex items-center justify-between gap-3 rounded-xl border bg-background/50 p-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Button size="icon" variant="ghost" onClick={() => toast.success(`Rappel WhatsApp envoyé à ${r.patient}`)}>
          ...
          <CardContent className="space-y-2">
            {toSendReviews.map((r) => (
              <div key={r.id} className="flex items-center justify-between gap-2 rounded-xl border p-2.5">
                ...
                <Button size="sm" variant="outline" onClick={() => toast.success("Lien envoyé")}>
